...
impl TextRasterizer {
    ...
    fn render_cid_direct(
        ...
        for (char_code, _bytes_consumed) in TextCharIter::new(bytes, Some(font_info)) {
            ...
            // Get Unicode character for word-space detection.
            ...
            // expressed. Gating the paint on `is_whitespace()` therefore made six
            // byte values unpaintable whatever glyph the font puts on them —
            // 0x09..0x0D and 0x20 through the ASCII pass-through, and 0xCA through
            // Mac Roman, where U+00A0 is the only whitespace in the table. A
            // re-indexed subset numbering its glyphs from 0x01 walks straight
            // through the first run, and the advance still ran, so the glyph left
            // a gap that reads as a space.
            //
            // A glyph that genuinely is whitespace has an empty outline, so
            // `outline_glyph` yields no path and painting it is already a no-op;
            // `gid != 0` still keeps .notdef out.
            if gid != 0 {
                let mut pb = PathBuilder::new();
                let mut builder = SkiaOutlineBuilder(&mut pb);
                if ttf_face
